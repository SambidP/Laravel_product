                ...
                <div class="card shadow-lg border-0 rounded-3">
                    <div class="card-header bg-primary text-white">
                        <h4 class="mb-0">Categories List
                            <a href="{{ url('category/create') }}" class="btn btn-outline-light float-end ms-1">Add
                                Category</a>
                                ...
                                @foreach ($categories as $category)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="{{ route('product.index') }}?category_id={{ $category->category_id }}"
                                                class="btn btn-sm btn-outline-secondary">View Products</a>
                                            <form action="{{ route('category.destroy', $category->category_id) }}"
                                                method="POST" class="d-inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger">Move to
                                                    Trash</button>
                                            </form>
                                        </td>
                                    </tr>
